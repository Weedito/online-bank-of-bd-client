            ...
            <div className="modal modal-bottom sm:modal-middle">
                ...
                <div className="flex flex-col w-full rounded-lg bg-white shadow-lg">
                    ...
                    <div className="p-6 w-full md:w-5/6 mx-auto flex flex-col justify-start">
                        ...
                        <p className="text-gray-700 text-xs md:text-base font-semibold mb-4">NID Number: {loan?.NIDno}</p>
                        <p className="text-gray-700 text-xs md:text-base font-semibold mb-4">Phone: {loan?.phone}</p>
                        <p className="text-gray-700 text-xs md:text-base font-semibold mb-4">Date of Birth: {loan?.DOB}</p>
                        <p className="text-gray-700 text-xs md:text-base font-semibold mb-4">Gender: {loan?.gender}</p>
                        <p className="text-gray-700 text-xs md:text-base font-semibold mb-4">Branch: {loan?.branch}</p>
                        <p className="text-gray-700 text-xs md:text-base font-semibold mb-4">Religion: {loan?.religion}</p>
                        <p className="text-gray-700 text-xs md:text-base font-semibold mb-4">Country: {loan?.country}</p>
                        <p className="text-gray-700 text-xs md:text-base font-semibold mb-4">Address: {loan?.address}</p>
                        <p className="text-gray-600 text-xs">Account Created {loan?.OpeningDate}</p>
                        <div className="flex justify-around items-center gap-2 p-5 w-full mx-auto">
                        <img className=" object-cover rounded w-3/6 mx-auto" src={loan?.NIDFrontSide && loan?.NIDFrontSide} alt="" />
                        <img className=" object-cover rounded w-3/6 mx-auto" src={loan?.NIDBackSide && loan?.NIDBackSide} alt="" />
                        </div>
                    </div>
                </div>
